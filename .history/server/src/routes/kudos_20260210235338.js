...
router.post('/submit', auth, async (req, res) => {
  try {
    ...
    // If toUserId is not a valid ObjectId, try to resolve by name or email
    const mongoose = require('mongoose');
    if (!mongoose.Types.ObjectId.isValid(resolvedToUserId)) {
      const User = require('../models/User');
      const maybeUser = await User.findOne({
        $or: [{ name: resolvedToUserId }, { email: resolvedToUserId }],
      });
      if (maybeUser) {
        resolvedToUserId = maybeUser._id;
      } else {
